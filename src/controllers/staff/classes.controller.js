 ...
 * class document.
 *
 * `stats.current` aggregates only across users currently assigned to the
 * class (User.classId), so transfers out of the class no longer affect the
 * current totals.
 *
 * `stats.session` aggregates across every ClassEnrollment row ever recorded
 * for this class, plus all successful payments tagged with this classId
 * (including learners who were promoted elsewhere).
 *
 * Response:
 *   {
 *     class: { ...class fields },
 ...
 *         totalPaid,         // paid by current students (each capped at fee)
 *         totalRemaining,    // totalExpected - totalPaid
 *       },
 *       session: {
 *         enrollmentCount,   // total ClassEnrollment rows
 *         studentCount,      // distinct users ever enrolled
 *         leftCount,         // enrolled users no longer assigned (promus)
 *         totalExpected,     // sum of snapshotted classFee per enrollment
 *         totalPaid,         // all successful payments for this classId
 *         totalRemaining,    // totalExpected - totalPaid
 *       },
 *       // Legacy flat keys (same shape as before) — kept for existing clients.
 *       studentCount,
 *       fullyPaidCount,
 *       partiallyPaidCount,
 *       unpaidCount,
 *       totalExpected,
 *       totalPaid,           // now mirrors session.totalPaid (encaissé classe)
 *       totalRemaining,
 *     }
 *   }
 */
const details = asyncHandler(async (req, res) => {
  ...
  const [rollup, enrollmentRollup, sessionTotalPaid] = await Promise.all([
    Payment.classRollup(studentObjectIds, cls._id, cls.fee),
    ClassEnrollment.rollupForClass(cls._id),
    Payment.classTotalPaid(cls._id),
  ]);
  ...
  let sessionStudentCount = enrollmentRollup.studentCount;
  let sessionTotalExpected = enrollmentRollup.totalExpected;
  let sessionLeftCount = enrollmentRollup.leftCount;
  let sessionEnrollmentCount = enrollmentRollup.enrollmentCount;

  // Users assigned before ClassEnrollment existed — treat current cohort as session minimum.
  if (sessionStudentCount === 0 && current.studentCount > 0) {
    sessionStudentCount = current.studentCount;
    sessionTotalExpected = current.totalExpected;
    sessionLeftCount = 0;
    sessionEnrollmentCount = current.studentCount;
  }

  const session = {
    enrollmentCount: sessionEnrollmentCount,
    studentCount: sessionStudentCount,
    leftCount: sessionLeftCount,
    totalExpected: sessionTotalExpected,
    totalPaid: sessionTotalPaid,
    totalRemaining: Math.max(0, sessionTotalExpected - sessionTotalPaid),
  };

  // Previous flat-only response (pre ClassEnrollment session stats):
  // return ok(res, {
  //   class: cls,
  //   stats: {
  //     studentCount: rollup.studentCount,
  ...
  return ok(res, {
    ...
    stats: {
      ...
      current,
      session,
      // Legacy flat keys — mirror current + session.totalPaid for old clients.
