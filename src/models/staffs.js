...
const userSchema = new Schema({
  ...
  status: {
    type: Number,
    default: STAFF_USER_STATUS.INACTIVE,
    enum: Object.values(STAFF_USER_STATUS) // 1 = inactive, 20 = active, -1 = suspended
  },
  role: {
    type: Number,
    default: STAFF_USER_ROLES.STAFF,
    enum: Object.values(STAFF_USER_ROLES)
  },
  },
  {
...
userSchema.statics.activateAccount = async function(userId){
  try{
    ...
    let user = await this.findOne({userId, isDeleted: false});
    if(user){
      user.status = STAFF_USER_STATUS.ACTIVE;
...
const model = mongoose.model('staff', userSchema);
